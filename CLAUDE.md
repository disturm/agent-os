...
- `lib/agent-result.ts` — типы ответа API и `VERDICT_META` (расшифровка вердикта + классы бейджа). Обе колонки читают вердикт отсюда, дублировать нельзя.
...
- Файлов и папок сверх структуры из spec2 не создавать. Исключения: служебные `app/layout.tsx`, `tsconfig.json`, `next.config.ts` и весь UI-слой (`app/globals.css`, `components/`, `lib/`, `components.json`, `postcss.config.mjs`) — он добавлен отдельным решением, см. раздел «Дополнение» в `spec2.md`.
