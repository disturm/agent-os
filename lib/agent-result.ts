 ...
 *
 * Пометка «MCP» — откуда инструмент пришёл: свой сервер поверх markdown или локальный
 * `tool()` в `src/skills/`. Для агента разницы нет, оба вызываются одинаково и лежат
 * в одном `toolCalls`; пометка нужна человеку, который смотрит на прогон.
 */
export const TOOL_META: Record<string, string> = {
  read_profile: 'прочитал профиль (MCP)',
  read_recent_logs: 'заглянул в дневник (MCP)',
  list_recipes: 'посмотрел рецепты (MCP)',
  save_health_plan: 'сохранил план (MCP)',
